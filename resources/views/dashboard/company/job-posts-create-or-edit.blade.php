            ...
            <div class="grid grid-cols-1">
                <div class="card dark:bg-zinc-800 dark:border-zinc-600">
                    <div class="card-body border-b border-gray-100 dark:border-zinc-600">
                        <h6 class="mb-1 text-gray-700 text-15 dark:text-gray-100">Job Post {{ $currentRouteName == 'company.job-posts.create' ? 'Create' : 'Update' }}</h6>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="col-span-12 lg:col-span-12">
                                ...
                                <form action="{{ $currentRouteName == 'company.job-posts.create' ? route('company.job-posts.store') : route('company.job-posts.update', $job_post->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <textarea id="job-description" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-white-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Write Job Title Description">
                                            {{ $currentRouteName == 'company.job-posts.edit' ? $job_post->description : '' }}
                                        </textarea>
                                    </div>
                                    <div class="mb-4">
                                        <label for="external-apply-link" class="block mb-2 font-medium text-gray-700 dark:text-gray-100">External Apply Link:</label>
                                        <input class="w-full placeholder:text-sm py-1.5 rounded border-gray-100 focus:border focus:border-violet-500 focus:ring focus:ring-violet-500/20 dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-100 dark:text-zinc-100" type="url" name="apply_link" value="{{ $currentRouteName == 'company.job-posts.edit' ? $job_post->apply_link : '' }}" placeholder="https://getbootstrap.com" id="external-apply-link">
                                    </div>
                                    <div class="mb-4">
                                        <label for="job-expiration-date" class="block mb-2 font-medium text-gray-700 dark:text-gray-100">Job Expiration Date:</label>
                                        <input class="w-full border-gray-100 rounded placeholder:text-sm focus:border focus:border-violet-500 focus:ring-0 dark:bg-zinc-700/50 dark:border-zinc-600 dark:text-zinc-100" type="date" name="job_expiration_date" value="{{ $currentRouteName == 'company.job-posts.edit' ? $job_post->job_expiration_date : '' }}" id="job-expiration-date">
                                    </div>
                                    <div class="mb-4">
                                        <label for="is-job-public" class="block mb-2 font-medium text-gray-700 dark:text-zinc-100">Is Job Public?</label>
                                        <select class="dark:bg-zinc-800 dark:border-zinc-700 w-full rounded border-gray-100 py-2.5 text-sm text-gray-500 focus:border focus:border-violet-500 focus:ring-0 dark:bg-zinc-700/50 dark:text-zinc-100" name="is_public">
                                            @if (isset($job_post))
                                                @if ($job_post->is_public)
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input class="w-full placeholder:text-13 text-13 py-1.5 rounded border-gray-100 focus:border focus:border-violet-50 focus:ring focus:ring-violet-500/20  dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-100 placeholder:text-gray-800 dark:text-zinc-100" type="text" name="location" value="{{ $currentRouteName == 'company.job-posts.edit' ? $job_post->location : '' }}" placeholder="Ka-115/6/1,Mohakhali Dakkhinpara" id="job-location">
                                    </div>
                                    <div class="mb-4">
                                        <label for="salary-range" class="block mb-2 font-medium text-gray-700 dark:text-gray-100">Salary Range:</label>
                                        <input class="w-full placeholder:text-13 text-13 py-1.5 rounded border-gray-100 focus:border focus:border-violet-50 focus:ring focus:ring-violet-500/20  dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-100 placeholder:text-gray-800 dark:text-zinc-100" type="text" name="salary_range" value="{{ $currentRouteName == 'company.job-posts.edit' ? $job_post->salary_range : '' }}" placeholder="100,000-150,000" id="salary-range">
                                    </div>
                                    <div class="mb-4">
                                        <label for="is-job-active" class="block mb-2 font-medium text-gray-700 dark:text-zinc-100">Is Job Active?</label>
                                        <select class="dark:bg-zinc-800 dark:border-zinc-700 w-full rounded border-gray-100 py-2.5 text-sm text-gray-500 focus:border focus:border-violet-500 focus:ring-0 dark:bg-zinc-700/50 dark:text-zinc-100" name="is_active">
                                            @if (isset($job_post))
